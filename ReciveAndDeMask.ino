...
void ReciveAndDemask(){                                       // variablerna man tar emot måste vara globala. 
  if (radio.available()) {
        unsigned long int message = 0;
    radio.read(&message, sizeof(message));
     int index= indexMask & message;
   ...
   //   Serial.println("jeystick1");
       unsigned long temp = joystick1XMask & message;
        joystick1Xval = (temp >> joystick1XShift);
                 temp = joystick1YMask & message;
    ...
    //   Serial.println("jeystick2");
        unsigned long int temp = joystick2XMask & message;
        joystick2Xval = (temp >> joystick2XShift);
                          temp = joystick2YMask & message;
